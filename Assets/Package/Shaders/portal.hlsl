...
// (i, 1) = x/y axis as f16x6 in u16x6 in u32x3. is_ellipse bit in w.
...
struct PortalPixel0 {
    ...
    static PortalPixel0 decode_crt(Texture2D<uint4> crt, uint index) { return decode_crt(crt[uint2(index, 0)]); }
};
...
struct Portal {
    ...
    static Portal decode_crt(PortalPixel0 pixel0, Texture2D<uint4> crt, uint index) { return decode_crt(pixel0, crt[uint2(index, 1)]); }
...
float u14_to_f16(uint input) { return (precise float) f16tof32(input & 0x00003fff); }
float3 u14_to_f16(uint3 input) { return (precise float3) f16tof32(input & 0x00003fff); }
...
static Portal Portal::decode_grabpass(float4 pixels[4]) {
    Portal p;
    uint3 position_bits = f16_to_u14(pixels[0].xyz) | (f16_to_u14(pixels[1].xyz) << 14) | (f16_to_u14(pixels[2].xyz) << 28);
