...
namespace LP {
    ...
    fixed4 f32_to_rgba_8unorm(float f) {
        const uint bits32 = asuint(f);
        const uint4 bits4x8 = (bits32 >> uint4(24, 16, 8, 0)) & 0xFF;
        return fixed4(float4(bits4x8) / 255.0); // make unorm
    }
    float rgba_8unorm_to_f32(fixed4 rgba) {
        const uint4 bits4x8 = uint4(ceil(255.0 * float4(rgba)));
        const uint4 shifted_bits4x8 = bits4x8 << uint4(24, 16, 8, 0);
        return asfloat(shifted_bits4x8[0] | shifted_bits4x8[1] | shifted_bits4x8[2] | shifted_bits4x8[3]);
    }
